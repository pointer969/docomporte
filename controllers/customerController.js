        ...
        .find({}, function(err, customers){
          Customer.count().exec(function(err, count){
              if (count > 0) {
                    ...
                    res.render('customers/new.jade', {title: 'DriveOn | Novo Cliente',baseuri:baseurl});
                  }     
            });        
        })
        .limit(limit)
 ...
 if (req.params.id != null || req.params.id != undefined) {      
  Customer.findOne({_id: req.params.id}).exec(function (err, customer) {
        if (err) {
          ...
          {
             case 11000: 
                 req.flash('alert-danger', 'Estes dados já existem no registro de Drivers.')    
                 break;        
             default: 
...
exports.save  =   function(req, res){
    ...
    customer.save(function(err) {
      if(err) {  
        ...
        res.render('customers/new', { title: 'DriveOn | Novo Cliente', baseuri:baseurl})
      } else {          
        req.flash('alert-info', 'Dados salvos com sucesso!')  
        res.redirect('/customers/show/'+customer._id)
      }
