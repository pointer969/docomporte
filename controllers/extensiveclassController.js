        ...
        .find({}, function(err, extensiveclasses){
          ExtensiveClass.count().exec(function(err, count){
              if (count > 0) {
                    ...
                    res.render('extensiveclasses/new.jade', { title: 'DriveOn | Novo Tipo',baseuri:baseurl});
                  }     
            });        
        })        
        .limit(limit)
...
extensiveclassController.edit = function(req, res){ 
  ...
  ExtensiveClass.findOne({_id: req.params.id}).exec(function (err, uprofile) {
        if (err) {
          ...
          {
             case 11000: 
                 req.flash('alert-danger', 'Estes dados já existem no registro de extensiveclasses.')    
                 break;        
             default: 
